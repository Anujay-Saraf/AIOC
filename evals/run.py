...
def _run_scenario(name: str, spec: Dict[str, Any]) -> EvalResult:
    ...
    checks = {
        ...
        "troubleshooting": all(
            term.lower() in " ".join(state.troubleshooting_plan).lower()
            for term in spec.get("required_troubleshooting_terms", [])
        ),
        "stakeholder_updates": all(
            term.lower() in " ".join(str(value) for value in state.stakeholder_updates.values()).lower()
            for term in spec.get("required_stakeholder_terms", [])
        ),
    }
