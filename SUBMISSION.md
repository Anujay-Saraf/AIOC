...
- **Local-first Ollama runtime with production fallback**: Jarvis prefers local Ollama, but the admin UI checks whether the selected model is installed and feasible on the host. If the local runtime is unavailable, the system prompts for an online provider API key and falls back to the next configured runtime instead of failing silently.
...
- **Multilingual voice-enabled incident copilot** with browser speech input/output and a local RAG knowledge base for grounded answer retrieval.
