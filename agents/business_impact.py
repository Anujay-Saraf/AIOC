...
def business_impact(state: IncidentState) -> IncidentState:
    t = Timer.begin()
    service_config: dict[str, Any] = get_telemetry_provider().service_config()
    ...
    cost_metric: Any = next(
        (
            m
            for m in state.raw_metrics
            if str(m.get("metric_name", "")).lower() in {"cost_per_minute", "infra_cost_per_minute", "request_cost_per_minute"}
            and str(m.get("window", "")).lower() == "incident"
        ),
        None,
    )
    ...
    state.estimated_cost_impact_per_minute = float(cost_metric.get("value", 0.0)) if cost_metric else 0.0
    ...
    state.revenue_impact_justification = {
        ...
        "estimated_cost_impact_per_minute": state.estimated_cost_impact_per_minute,
    ...
    # --- Non-blocking knowledge-graph note on historical revenue impact ------
    try:
        kg_result = kg_query(
            f"historical revenue impact of similar incidents for {state.service}",
            incident_context={
        ...
        state.historical_impact_note = (
            "Historical similar-incident impact: "
