...
class SlackAssistant:
    ...
    def __init__(self) -> None:
        ...
        self.public_base_url = (
            ...
            or os.getenv("WEB_BASE_URL", "").rstrip("/")
            or "http://localhost:8000"
        )
        self.web_base_url = os.getenv("WEB_BASE_URL", "").rstrip("/")
    ...
    def incident_url(self, incident_id: str) -> str:
        base_url = self.web_base_url or self.public_base_url
        return f"{base_url}/incident/{incident_id}"
...
def summarize_incident(record: Dict[str, Any]) -> str:
    ...
    cost_impact = float(record.get("estimated_cost_impact_per_minute", 0.0) or 0.0)
    text = (
        ...
        f"${record.get('estimated_revenue_impact_per_minute', 0):.2f}/min revenue"
        + (f", ${cost_impact:.2f}/min cost" if cost_impact else "")
    )

    # --- Non-blocking knowledge-graph: prepend a high-confidence similar ------
    # incident line when posting an update, so responders see prior art at a glance.
    try:
        kg_result = kg_query(
            f"similar past incident for {record.get('service', 'unknown')}",
            incident_context={
        ...
        for fact in kg_result.get("facts", []):
            if fact.get("confidence", 0.0) >= 0.7:
                text = (
                    f"_Similar past incident:_ {fact['fact']} (via {fact['citation']})\n" + text
                )
                break
    except Exception:
        pass
...
def incident_blocks(record: Dict[str, Any], assistant: SlackAssistant) -> List[Dict[str, Any]]:
    ...
    troubleshooting = record.get("troubleshooting_plan", [])[:2]
    if troubleshooting:
        blocks.append(
            {
                "type": "section",
